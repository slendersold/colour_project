...
class ImageAlignment:
    ...
    def give_original_coordinates(self, coordinates, w, h, type):
        ...
        def rotate_coordinates(coordinates, width, height, theta, type):
            ...
            if type == 'circle':
                for key, value in coordinates.items():
                    x_new, y_new = rotate_coordinate(value['x_centroid'], value['y_centroid'], cx, cy, theta)
                    rotated_dict[key] = {
                        'x_centroid': x_new,
                        'y_centroid': y_new,
                        'radius': value['radius']
            ...
            if type == 'rectangle':
                for key, value in coordinates.items():
                    x0_new, y0_new = rotate_coordinate(value['x0'], value['y0'], cx, cy, theta)
                    x1_new, y1_new = rotate_coordinate(value['x1'], value['y1'], cx, cy, theta)
                    rotated_dict[key] = {
                        'y0': y0_new,
                        'y1': y1_new,
                        'x0': x0_new,
        ...
        if self.position.get('flip_horizontal'):
            ...
            coordinates_unsettled = flip_coordinates_horizontal(coordinates_unsettled, w, type)
        if self.position.get('flip_vertical'):
            print('Flip vertical')
            coordinates_unsettled = flip_coordinates_vertical(coordinates_unsettled, h, type)
        if self.position.get('flip_over'):
            print('Flip over')
            coordinates_unsettled = rotate_coordinates(coordinates_unsettled, w, h, 180, type)
        ...
        if rotation_angle != 0:
            ...
            coordinates_unsettled = rotate_coordinates(coordinates_unsettled, w, h, rotation_angle, type)
