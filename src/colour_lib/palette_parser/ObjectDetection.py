...
class ObjectDetection:
    ...
    def find_rectangles(self, image_to_find, image_to_show, show_image=True):
        ...
        # 1. FIND CONTOURS
        contours, _ = cv2.findContours(image_to_find, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def find_circles(self, image, averaging_threshold):
        ...
        width = image.shape[1]
        circles = [contour for contour in contours if is_circle(contour=contour, width=width, tolerance=0.2)]
