...
class StackedRegressor:
    def __init__(self, train_data, reference_data, random_state=None):
        # Базовые модели
        self.base_models = [
            ("rf", MultiOutputRegressor(RandomForestRegressor(random_state=random_state))),
            ("gbr", MultiOutputRegressor(GradientBoostingRegressor(random_state=random_state))),
        ...
        self.meta_model_coefs = [estimator.coef_ for estimator in self.meta_model.estimators_]
