...
class VoteRegressor():
  def __init__(self, train_data, reference_data, random_state):
    ...
    # reg3 = PLSRegression()
    base_models = [('linear', LinearRegression()),('rf', RandomForestRegressor()),('svr', SVR())]
    ...
    self.reg["red reg"].fit(train_data[:,0].reshape(-1, 1), reference_data[:,0].reshape(-1, 1))
    self.reg["green reg"].fit(train_data[:,1].reshape(-1, 1), reference_data[:,1].reshape(-1, 1))
    self.reg["blue reg"].fit(train_data[:,2].reshape(-1, 1), reference_data[:,2].reshape(-1, 1))
